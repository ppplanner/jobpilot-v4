...
> 本文件由 Codex 每次会话自动加载。**文末含「改动日志」,每次改动后必须更新一条。**
...
### 2026-07-09
- **首页模块编号样式对齐投递看板**:按用户反馈,首页小标题前的编号部分改用 `SheetCode`,恢复与投递看板一致的等宽字体、10px、uppercase、字距样式;模块标题文字继续保留 `font-cartoon`。
- **首页整体铺浅格线底图**:按用户反馈,将首页根内容区加上与投递看板同源的 `SHEET_GRID` 浅格线底图,不再只在单个工作卡内出现;卡片仍保留白底和边框承载内容,避免文字直接压在格线背景上。
- **首页标题字体/白底线框/浅格底调整**:按用户反馈,恢复首页模块小标题 `font-cartoon` 字体,避免上轮对齐投递看板时把标题字体删掉;首页所有外层工作卡改为 `WorkCard`,复用投递看板同源 `SHEET_GRID` 浅格子底图,让白底页面更丰富;内部上传区、文本域、素材行、历史版本卡、JD 输入、信息标签改为白色填充,主要依靠边框/选中描边区分状态,减少浅绿色填充面积。
- **首页小标题与字号进一步对齐投递看板**:按用户反馈,首页取消独立英文 eyebrow、Next Step 说明卡和 10px 微字号;模块标题改成投递看板式“编号 + 标题 + 右侧计数”结构,但本地控制为三档字号:页面标题 `text-xl`,模块标题/正文 `text-sm`,辅助信息/标签 `text-xs`。卡内内容去掉多余 `mt-3` 层级,更像投递看板的轻量列表与表单格式。
- **首页样式收敛到现有功能页体系**:按用户反馈,将新首页双区工作台的视觉层级与投递看板/简历工作台看齐:页面标题降回 `text-xl` + `text-xs` 辅助说明,卡片统一 `rounded-xl border bg-[var(--surface)] p-4`,去掉独立的 `shadow-sm` 和深色说明卡,输入区统一 `bg-[var(--surface2)] rounded-lg`,按钮回到项目内常用小号主按钮。仅调整前端样式,不改首页功能链路。
- **首页改为双区简历适配工作台**:按用户新参考图和产品目标,不再让首页直接复用 `/resume` 整页,改成首页专用工作台:左侧为基础简历上传/粘贴、素材库集合选择、历史简历横向入口;右侧为目标 JD 粘贴、公司/岗位信息、能力标签与最近 JD 要求概览。历史简历仍链接 `/resume?version_id=...` 还原上次修改情况;首页点击“开始匹配修改”会把基础简历/JD/公司/岗位写入 localStorage 并跳 `/resume?from_home=1`,由原简历工作台继续完成 JD 解读、素材推荐、AI 改写和保存版本。仅改前端 `frontend/app/page.tsx` 与 `frontend/app/resume/page.tsx`,未动后端/数据库。

### 2026-07-01
- **高保真样式稿 v3 简历适配蓝图**:按用户反馈,v2 过于完整套用建筑事务所网站风格,无法体现产品自身特点。本次将 `frontend/public/jobpilot-hifi-style.html` 调整为“简历适配蓝图”:首页逻辑从功能卡片改为“基础简历底图→能力图层→JD 约束图层→匹配修改方案”,视觉只借建筑规划里的底图/图层/批注/修订语言,不再整体套建筑网站皮肤。仍未改动真实业务页面、接口或数据库。
- **高保真样式稿 v2 转建筑设计风格**:按用户反馈“AI 味道主要来自图框和颜色风格”,在不改动业务页面代码的前提下,直接更新 `frontend/public/jobpilot-hifi-style.html` 的视觉系统:从圆角 SaaS 卡片转为建筑事务所网站/规划展板语言,采用石材底色、黑色细线框、无圆角容器、图纸网格、项目编号、陶土强调色和更克制的状态标签,用于验证后续首页与简历工作台真实前端改造方向。
- **新增高保真样式稿**:基于《前端产品形态与交互说明.md》和现有前端布局约束,新增 `frontend/public/jobpilot-hifi-style.html`,产出 3 张 1440px 桌面高保真画板：首页工作台、简历工作台 Step2、简历工作台 Step3。目标是在不改动当前前端页面代码/接口/数据库的前提下,先把“AI 助手感”收敛为“转行产品经理简历适配工作台”的 Figma 可参考视觉方案,明确色卡、组件、文案替换和落地规范。

### 2026-06-29(晚)
- **微调图标/插画为建筑规划分析图风格**:参考用户给的图,不动任何模块布局,只统一视觉语言:侧边栏 Logo/角色标识从 emoji 改为深蓝线稿+橙色节点的文档适配标;首页能力标签行的方块图标改为规划分析式人物,并区分指向节点/读图纸/拿文件行走/标注面板等动作;历史简历版本卡上半区从 emoji 档位图改为“图纸+差异化人物+橙色节点”的小插画;简历工作台 Hero 插画追加两个带明确行为的线稿人物(拿图纸、标注面板)。目标是更接近“转行新人简历适配实验室/规划分析图”的主题。
- **能力匹配面板回退精简 + 对齐日历**:按用户反馈,删除首页「能力匹配面板」顶部“我的能力标签 × 岗位要求”统计说明区及我的标签/JD要求/命中/待补齐四个统计块,恢复为左右双列标签列表;能力卡增加固定高度与内部滚动,默认状态下底部与右侧「求职日程」卡底部对齐,避免统计信息挤占首页空间。仅改 `frontend/app/page.tsx`,未动接口/数据库。`npx tsc --noEmit --pretty false` 已通过。
- **产品定位转向“转行新人多 JD 适配”试改**:不再走“求职陪伴/可爱 IP”主轴,先把核心视觉和文案收回到“基础简历→多个 JD→能力标签匹配→岗位缺口补齐”。`resume/page.tsx` 顶部改为“转行新人多 JD 简历适配台”Hero,新增代码内信息型 SVG 插画(基础简历、目标 JD、标签连线、多版本适配),步骤文案改为“基础简历 / 目标 JD / 生成版本”。首页 `page.tsx` 的入口卡改名“JD 简历适配台”,能力区从“能力胶囊”升级为“能力匹配面板”,新增我的标签/JD要求/命中/待补齐统计,突出“我的能力标签 × 岗位要求”的产品主轴。未动后端、数据库和标签接口。
...
### 2026-06-24
- **新增**:UI 截图工具 `tools/screenshot.py`(Playwright)。Codex 可自行截图查看页面渲染效果(支持指定 url / 视口 / 全页 / `--ls=` 预设 localStorage 跳过弹窗),用于 UI 迭代自查,无需每次让用户截图。
